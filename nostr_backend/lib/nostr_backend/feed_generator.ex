defmodule NostrBackend.FeedGenerator do
  ...
  def init(_) do
    Logger.warning(
      "FeedGenerator GenServer starting (this should only happen once unless there is a crash or code reload)"
    )
  ...
  def handle_info({:EXIT, pid, reason}, state) do
    Logger.error(
      "Linked process #{inspect(pid)} exited: #{inspect(reason)} (ignored, FeedGenerator will not crash)"
    )
  ...
  defp generate_all do
    try do
      ...
      e ->
        Logger.error(
          "Exception in generate_all: #{inspect(e)}\n" <>
  ...
  defp safe_follow_list(source_pubkey) do
    try do
      ...
      e ->
        Logger.error(
          "Exception in FollowListCache.get_follow_list: #{inspect(e)}\n" <>
            Exception.format(:error, e, __STACKTRACE__)
        )
  ...
  defp safe_fetch_all_articles(follow_list) do
    try do
      ...
      e ->
        Logger.error(
          "Exception in fetch_all_articles: #{inspect(e)}\n" <>
            Exception.format(:error, e, __STACKTRACE__)
        )
  ...
  defp generate_language_feed(articles, language_code, feed_title) do
    ...
    if length(language_articles) > 0 do
      ...
      most_recent_date =
        if length(sorted_articles) > 0 do
      ...
      # Create entries
      entries =
        Enum.map(sorted_articles, fn article ->
      ...
      xml =
        Atomex.Feed.new(feed_id, most_recent_date, title)
      ...
      Logger.info(
        "Generated language feed for #{language_code} with #{length(sorted_articles)} articles"
      )
  ...
  defp generate_main_feed(articles) do
    ...
    xml =
      Atomex.Feed.new(feed_id, most_recent_date, "Pareto Articles")
  ...
  defp article_to_feed_entry(article) do
    ...
    if is_nil(author) do
      ...
      author_profile =
        case NostrBackend.ProfileCache.get_profile(author, []) do
          {:ok, profile} ->
  ...
  defp generate_sitemaps(articles) do
    ...
    with {:ok, follow_list} <- FollowListCache.get_follow_list(source_pubkey) do
      ...
      if map_size(articles_by_year) == 0 do
        ...
        year_sitemaps =
          Enum.map(articles_by_year, fn {year, year_articles} ->
            ...
            urls_xml =
              Enum.map(sorted_articles, fn article ->
                ...
                Logger.debug(
                  "Creating sitemap entry for article: kind=#{kind}, author=#{author}, identifier=#{identifier}"
                )
...
#{sitemap_entries}
  ...
  defp generate_author_sitemap(follow_list) do
    ...
    urls_xml =
      Enum.map(follow_list, fn pubkey ->
...
#{urls_xml}
  ...
  defp group_articles_by_year(articles) do
    ...
    |> Enum.filter(fn article ->
      case article.published_at do
        ...
        _ ->
          Logger.warning(
            "Article has invalid published_at format: #{inspect(article.published_at)}"
          )
  ...
  defp alternate_links(path) do
    ...
    codes =
      NostrBackend.Locale.available_languages()
      |> Enum.map(fn code ->
