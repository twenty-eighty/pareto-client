...
import Crypto exposing (chacha20, hmacSha256, sha256)
...
pad plaintext =
    let
        ...
        prefix =
            Bytes.Encode.unsignedInt16Be unpaddedLen

        suffix =
            Bytes.Encode.repeat (paddedLen - unpaddedLen) 0
    in
...
unpad padded =
    case Bytes.Decode.decode Bytes.Decode.unsignedInt16Be padded of
        ...
        Ok ( unpaddedLen, rest ) ->
            let
                unpadded =
                    Bytes.Decode.bytes unpaddedLen rest
            in
...
decodePayload payload =
    ...
    if plen == 0 || Bytes.get 0 payload == Just (toByte '#') then
        ...
        let
            ...
            nonce =
                Bytes.slice 1 33 payload

            ciphertext =
                Bytes.slice 33 (plen - 32) payload

            mac =
                Bytes.slice (plen - 32) plen payload
        in
        case vers of
            Just 2 ->
...
getMessageKeys conversationKey nonce =
    if Bytes.length conversationKey /= 32 then
        ...
        let
            keys =
                hkdfExpand conversationKey nonce 76

            chachaKey =
                Bytes.slice 0 32 keys

            chachaNonce =
                Bytes.slice 32 44 keys

            hmacKey =
                Bytes.slice 44 76 keys
        in
        ...
        Ok ( chachaKey, chachaNonce, hmacKey ) ->
            case pad plaintext of
                ...
                Ok padded ->
                    let
                        ciphertext =
                            chacha20 chachaKey chachaNonce padded

                        mac =
                            hmacAad hmacKey ciphertext nonce
                    in
                    case mac of
                        Err err ->
                            Err err
            ...
            case getMessageKeys conversationKey nonce of
                ...
                Ok ( chachaKey, chachaNonce, hmacKey ) ->
                    ...
                    if calculatedMac /= Ok mac then
                        ...
                        let
                            paddedPlaintext =
                                chacha20 chachaKey chachaNonce ciphertext
                        in
